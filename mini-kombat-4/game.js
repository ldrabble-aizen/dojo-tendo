...
function idleBodyProfile(f) {
  return {
    p1: { weight: 1.14, sway: 0.52, arm: 0.66, guard: 0.34, bounce: 0.35, wide: 1.12, shoulder: 0.82, hip: 1.14, hand: 0.72, foot: 1.14, breath: 0.86, tempo: 0.9 },
    p2: { weight: 0.78, sway: 0.86, arm: 1.08, guard: 0.62, bounce: 0.72, wide: 0.86, shoulder: 1.12, hip: 0.82, hand: 1.16, foot: 0.82, breath: 1.14, tempo: 1.18 },
    p3: { weight: 1.28, sway: 0.38, arm: 0.58, guard: 0.3, bounce: 0.24, wide: 1.24, shoulder: 0.74, hip: 1.26, hand: 0.66, foot: 1.26, breath: 0.78, tempo: 0.78 },
    p4: { weight: 0.82, sway: 1.08, arm: 1.12, guard: 0.68, bounce: 0.9, wide: 0.82, shoulder: 1.12, hip: 0.82, hand: 1.18, foot: 0.84, breath: 1.2, tempo: 1.24 },
    p5: { weight: 0.7, sway: 0.98, arm: 1.18, guard: 0.56, bounce: 0.78, wide: 0.76, shoulder: 1.18, hip: 0.78, hand: 1.24, foot: 0.78, breath: 1.12, tempo: 1.18 },
    p6: { weight: 1.02, sway: 0.5, arm: 0.72, guard: 0.78, bounce: 0.28, wide: 1.02, shoulder: 0.92, hip: 1.02, hand: 0.82, foot: 1.04, breath: 0.92, tempo: 0.92 },
  }[f?.profileId] ?? { weight: 1, sway: 0.68, arm: 0.82, guard: 0.48, bounce: 0.5, wide: 1, shoulder: 1, hip: 1, hand: 1, foot: 1, breath: 1, tempo: 1 };
}

function idleVisualMotion(f) {
  const style = idleBodyProfile(f);
  const active = !winner && f?.grounded && (f?.hurt ?? 0) <= 0 && !f?.attack && !f?.blocking && Math.abs(f?.vx ?? 0) <= 0.45 ? 1 : 0;
  const clock = roundFrame + (f?.profileId?.charCodeAt(1) ?? 0) * 17 + (f?.x ?? 0) * 0.04;
  const tempo = style.tempo ?? 1;
  const breath = Math.sin(clock * 0.055 * tempo);
...
function guardCommitmentProfile(f) {
  return {
    p1: { absorb: 1.18, snap: 0.78, hand: 0.88, foot: 1.18, shoulder: 0.9, counter: 0.86 },
    p2: { absorb: 0.82, snap: 1.22, hand: 1.2, foot: 0.84, shoulder: 1.18, counter: 1.2 },
    p3: { absorb: 1.32, snap: 0.68, hand: 0.76, foot: 1.3, shoulder: 0.74, counter: 0.78 },
    p4: { absorb: 0.86, snap: 1.18, hand: 1.16, foot: 0.88, shoulder: 1.18, counter: 1.16 },
    p5: { absorb: 0.78, snap: 1.26, hand: 1.24, foot: 0.8, shoulder: 1.24, counter: 1.24 },
    p6: { absorb: 1.06, snap: 0.92, hand: 0.96, foot: 1.06, shoulder: 0.96, counter: 0.98 },
  }[f?.profileId] ?? { absorb: 1, snap: 1, hand: 1, foot: 1, shoulder: 1, counter: 1 };
}

function guardPresentationMotion(f) {
  const guardProfile = characterGuard(f);
  const absorption = guardAbsorptionProfile(f);
  const commitment = guardCommitmentProfile(f);
  ...
  const guardRaw = available && f?.blocking ? clamp((f?.guardPulse ?? 0) / 18, 0, 1) : 0;
  const entryRaw = available ? clamp((f?.guardEntryPulse ?? 0) / 12, 0, 1) : 0;
  const exitRaw = available ? clamp((f?.guardExitPulse ?? 0) / 10, 0, 1) : 0;
  const impactRaw = available ? clamp((f?.guardImpact ?? 0) / GUARD_IMPACT_FRAMES, 0, 1) : 0;
  const counterRaw = available ? clamp((f?.counterWindow ?? 0) / COUNTER_WINDOW_FRAMES, 0, 1) : 0;
  const guard = smoothStep01(Math.max(guardRaw, entryRaw * 0.7));
  const entry = smoothStep01(entryRaw);
  const exit = smoothStep01(exitRaw);
  const impact = smoothStep01(impactRaw);
  const counter = smoothStep01(counterRaw);
  const counterSnap = Math.sin((1 - counterRaw) * Math.PI) * counter * counterProfile.drive;
  const counterCoil = counter * counterProfile.coil;
  const brace = clamp((guard * 0.78 + entry * 0.46 + impact * 1.16) * commitment.absorb, 0, 1.95);
  const recoil = clamp(impact * guardProfile.recoil * absorption.twist, 0, 1.75);
  const shoulderSink = clamp((brace * 0.58 + impact * 0.72) * commitment.shoulder, 0, 1.85);
  const hipSet = clamp((brace * 0.74 + impact * 0.38 + counterCoil * 0.22) * absorption.hip, 0, 1.9);
  const footPress = clamp((brace * 0.72 + impact * 0.55 + counterCoil * 0.34) * commitment.foot * absorption.foot, 0, 1.9);
  const footSpread = clamp((guardProfile.crouch * 0.28 + brace * 0.44 + counterCoil * 0.22) * commitment.foot, 0, 1.55);
  const handGuard = clamp((guard * 0.82 + entry * 0.62 + counter * 0.34) * guardProfile.handLift * commitment.hand, 0, 1.75);
  const handImpact = clamp((impact * 1.05 + counterSnap * 0.36) * absorption.hand, 0, 1.7);
...
function hitReactionAnatomyProfile(f) {
  return {
    p1: { shoulder: 0.86, hip: 1.14, armLag: 0.78, footLag: 0.9, settle: 1.18, twist: 0.82, handWhip: 0.78, kneeWhip: 0.92, spine: 0.82, fingerShock: 0.82, toeClaw: 1.16, crease: 0.9, shadowWeight: 1.18 },
    p2: { shoulder: 1.16, hip: 0.82, armLag: 1.24, footLag: 1.18, settle: 0.82, twist: 1.18, handWhip: 1.28, kneeWhip: 1.16, spine: 1.18, fingerShock: 1.26, toeClaw: 0.86, crease: 1.14, shadowWeight: 0.82 },
    p3: { shoulder: 0.74, hip: 1.28, armLag: 0.66, footLag: 0.78, settle: 1.32, twist: 0.72, handWhip: 0.66, kneeWhip: 0.82, spine: 0.74, fingerShock: 0.74, toeClaw: 1.3, crease: 0.84, shadowWeight: 1.32 },
    p4: { shoulder: 1.2, hip: 0.86, armLag: 1.18, footLag: 1.24, settle: 0.88, twist: 1.22, handWhip: 1.18, kneeWhip: 1.24, spine: 1.16, fingerShock: 1.18, toeClaw: 0.88, crease: 1.16, shadowWeight: 0.88 },
    p5: { shoulder: 1.26, hip: 0.76, armLag: 1.3, footLag: 1.22, settle: 0.78, twist: 1.28, handWhip: 1.34, kneeWhip: 1.28, spine: 1.22, fingerShock: 1.3, toeClaw: 0.78, crease: 1.2, shadowWeight: 0.78 },
    p6: { shoulder: 0.96, hip: 1.08, armLag: 0.9, footLag: 0.92, settle: 1.08, twist: 0.9, handWhip: 0.92, kneeWhip: 0.94, spine: 0.94, fingerShock: 0.96, toeClaw: 1.06, crease: 0.98, shadowWeight: 1.06 },
  }[f?.profileId] ?? { shoulder: 1, hip: 1, armLag: 1, footLag: 1, settle: 1, twist: 1, handWhip: 1, kneeWhip: 1, spine: 1, fingerShock: 1, toeClaw: 1, crease: 1, shadowWeight: 1 };
}

function entrancePoseProfile(f) {
  return {
    p1: { slide: 1.12, lift: 0.82, weight: 1.18, flourish: 0.86, lean: 0.025, guard: 0.74, settle: 1.16, foot: 1.16, front: [[45, -112], [48, -92]], back: [[-42, -118], [-35, -98]], aura: 0.82 },
    p2: { slide: 0.84, lift: 1.18, weight: 0.9, flourish: 1.12, lean: -0.035, guard: 1.16, settle: 0.86, foot: 0.82, front: [[42, -121], [48, -142]], back: [[-36, -98], [-28, -82]], aura: 1.18 },
    p3: { slide: 1.2, lift: 0.72, weight: 1.28, flourish: 0.72, lean: 0.038, guard: 0.66, settle: 1.3, foot: 1.28, front: [[43, -105], [55, -86]], back: [[-45, -116], [-40, -94]], aura: 0.72 },
    p4: { slide: 0.88, lift: 1.14, weight: 0.88, flourish: 1.18, lean: -0.028, guard: 1.12, settle: 0.9, foot: 0.86, front: [[45, -122], [58, -132]], back: [[-33, -106], [-42, -84]], aura: 1.12 },
    p5: { slide: 0.78, lift: 1.24, weight: 0.78, flourish: 1.24, lean: -0.02, guard: 1.18, settle: 0.82, foot: 0.78, front: [[39, -124], [55, -137]], back: [[-30, -104], [-39, -86]], aura: 1.2 },
    p6: { slide: 1.02, lift: 0.9, weight: 1.08, flourish: 0.94, lean: 0.012, guard: 0.92, settle: 1.06, foot: 1.04, front: [[40, -118], [47, -106]], back: [[-38, -116], [-30, -96]], aura: 0.94 },
...
function pivotVisualMotion(f) {
  const style = movementPivotProfile(f);
  const active = f?.grounded && (f?.hurt ?? 0) <= 0 && !f?.attack && !winner ? 1 : 0;
  const stopRaw = active ? clamp((f?.walkStopPulse ?? 0) / 16, 0, 1) : 0;
  const moveRaw = active ? clamp((f?.moveTurnPulse ?? 0) / 10, 0, 1) : 0;
  const faceRaw = active ? clamp((f?.facingTurnPulse ?? 0) / 14, 0, 1) : 0;
  const stopSpeed = clamp(f?.walkStopSpeed ?? 0.45, 0, 1.55);
  const stop = smoothStep01(stopRaw) * stopSpeed * style.brake;
  const stopRebound = Math.sin((1 - stopRaw) * Math.PI) * stopSpeed * style.snap;
  const moveTurn = smoothStep01(moveRaw) * style.brake;
  const moveSnap = Math.sin((1 - moveRaw) * Math.PI) * style.snap;
  const faceTurn = smoothStep01(faceRaw) * style.brake;
  const faceSnap = Math.sin((1 - faceRaw) * Math.PI) * style.snap;
  const stopDir = f?.walkStopDir || Math.sign(f?.vx ?? 0) || f?.dir || 1;
  const moveDir = f?.moveIntent || Math.sign(f?.vx ?? 0) || f?.dir || 1;
  const faceDir = f?.facingTurnTo || f?.dir || 1;
  const pivotSide = (f?.walkAnchorSide || f?.footPlantSide || 1) * (f?.dir || 1);
  const press = clamp(Math.max(stop * 0.82, moveTurn * 0.72, faceTurn * 0.92), 0, 1.7);
  const snap = clamp(Math.max(stopRebound * 0.52, moveSnap * 0.48, faceSnap * 0.62), 0, 1.55);
  const activeAmount = Math.max(press, snap);
  return { active, style, stopRaw, moveRaw, faceRaw, stopSpeed, stop, stopRebound, moveTurn, moveSnap, faceTurn, faceSnap, stopDir, moveDir, faceDir, pivotSide, press, snap, activeAmount };
}
...
function startAttack(f, type) {
  ...
  if (f.grounded && (heavy || sweep || grab)) {
    const supportSide = grab || sweep ? 1 : -1;
    f.footPlantSide = supportSide;
    f.walkAnchorSide = supportSide;
    f.footPlantPulse = Math.max(f.footPlantPulse ?? 0, grab ? 11 : heavy ? 12 : 10);
    f.walkAnchorPulse = Math.max(f.walkAnchorPulse ?? 0, grab ? 13 : heavy ? 14 : 12);
    f.walkPushPulse = Math.max(f.walkPushPulse ?? 0, grab ? 8 : heavy ? 9 : 7);
  }
...
function attackWhoosh(f) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    particles.push({
      ...
      length: fighterScale((spec.sweep ? 60 : spec.kick ? 72 : spec.special ? 66 : spec.grab ? 58 : 48) * (0.86 + Math.random() * 0.25)),
      life: life + Math.random() * 5,
      maxLife: life + 4,
      size: spec.kick || spec.sweep ? 3.8 : spec.special ? 3.5 : spec.grab ? 3.2 : 2.7,
      color: spec.core,
      kind: "impactNeedle",
    });
  }
...
function reactionPresentationProfile(f) {
  const mass = hitReactionMassProfile(f);
  const localized = localizedImpactProfile(f);
  const anatomy = hitReactionAnatomyProfile(f);
  const active = Math.max(mass.active, localized.t * 0.82);
  const zone = mass.zone === "head" || mass.zone === "legs" ? mass.zone : "torso";
  const kind = mass.kind || "light";
  const worldDir = mass.worldDir || f.impactDir || f.dir || 1;
  const localDir = worldDir === (f.dir || 1) ? 1 : -1;
  if (active <= 0.012 || kind === "guard") {
    return {
      active: 0,
      zone,
      kind,
      ...
      follow: 0,
      contactX: 0,
      contactY: zone === "head" ? -140 : zone === "legs" ? -46 : -100,
      handShock: 0,
      toeShock: 0,
      creaseAlpha: 0,
      shadowWeight: 0,
  ...
  const heavy = kind === "finish" || kind === "counter" || kind === "blast" || kind === "heavy";
  const zoneWeight = zone === "head" ? 1.08 : zone === "legs" ? 1.14 : 1;
  const force = clamp(
    (mass.snap * 0.46 + mass.rebound * 0.3 + mass.floor * 0.16 + mass.skid * 0.12 + localized.snap * 0.22) *
      (heavy ? 1.12 : 0.9) *
      zoneWeight,
    0,
    1.75
  ...
  const follow = clamp(
    Math.sin((1 - clamp(active, 0, 1)) * Math.PI) * clamp(mass.strength, 0.35, 1.9) * (heavy ? 1.12 : 0.86),
    0,
    1.45
  );
  ...
  return {
    ...
    contactX,
    contactY,
    handShock: clamp((force * 0.55 + follow * 0.32 + localized.t * 0.12) * anatomy.fingerShock, 0, 1.18),
...
function drawWorldContactShadow(f, baseX, walking, stride, impactEase) {
  ...
  if (facingTurn > 0.04) {
    ...
    const twist = facingPress + facingSnap * 0.42;
    ctx.strokeStyle = `rgba(255, 229, 151, ${0.08 + twist * 0.1})`;
    ctx.lineWidth = 1.5 + twist * 2.2;
    ctx.beginPath();
    ctx.ellipse(baseX - turnDir * (14 + facingPress * 8) * FIGHTER_SCALE, FLOOR + 7, 36 + twist * 26, 5.2 + twist * 3.2, -turnDir * 0.08, Math.PI * 0.05, Math.PI * 1.72);
    ctx.stroke();
    ctx.strokeStyle = colorWithAlpha(f.trim ?? "#fff1bd", 0.08 + facingSnap * 0.11);
    ctx.lineWidth = 1.1 + facingSnap * 1.5;
    ctx.beginPath();
    ctx.moveTo(baseX + turnDir * 18 * FIGHTER_SCALE, FLOOR + 6);
    ctx.quadraticCurveTo(baseX - turnDir * (8 + facingSnap * 18) * FIGHTER_SCALE, FLOOR + 12, baseX - turnDir * (46 + facingPress * 18) * FIGHTER_SCALE, FLOOR + 7);
    ctx.stroke();
    ctx.restore();

    ctx.save();
    ctx.globalCompositeOperation = "multiply";
    ctx.fillStyle = `rgba(45, 28, 15, ${0.075 + facingPress * 0.06})`;
    for (const side of [-1, 1]) {
      ctx.beginPath();
      ctx.ellipse(
        baseX + side * (24 + facingSnap * 8) * FIGHTER_SCALE * (f.dir || 1),
        FLOOR + 8,
        18 + facingPress * 11,
        4.2 + facingPress * 1.8,
        side * 0.06 - turnDir * 0.04,
        0,
        Math.PI * 2
      );
      ctx.fill();
  ...
  if (pivot.activeAmount > 0.045) {
    const pivotDir = pivot.faceRaw > 0.03 ? pivot.faceDir : pivot.stopRaw > 0.03 ? pivot.stopDir : pivot.moveDir;
    const pivotSide = pivot.pivotSide || 1;
    const releaseSide = -pivotSide;
    const press = clamp(pivot.press, 0, 1.55);
    const snap = clamp(pivot.snap, 0, 1.35);
    const pivotX = baseX + pivotSide * (21 + press * 9) * FIGHTER_SCALE;
    const releaseX = baseX + releaseSide * (18 + snap * 8) * FIGHTER_SCALE;

    ctx.save();
    ctx.globalCompositeOperation = "multiply";
    ctx.fillStyle = `rgba(45, 27, 14, ${0.052 + press * 0.052})`;
    ctx.beginPath();
    ctx.ellipse(pivotX, FLOOR + 8, 18 + press * 15, 4 + press * 2.1, -pivotDir * 0.045, 0, Math.PI * 2);
    ctx.fill();
    if (snap > 0.04) {
      ctx.fillStyle = `rgba(53, 32, 17, ${0.025 + snap * 0.042})`;
      ctx.beginPath();
      ctx.ellipse(releaseX, FLOOR + 7, 14 + snap * 10, 3.5 + snap * 1.5, pivotDir * 0.06, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
    ...
    ctx.save();
    ctx.globalCompositeOperation = "screen";
    ctx.strokeStyle = colorWithAlpha(f.trim ?? "#fff1bd", 0.045 + press * 0.055 + snap * 0.035);
    ctx.lineWidth = 1 + press * 0.9 + snap * 0.45;
    ctx.beginPath();
    ctx.ellipse(pivotX, FLOOR + 6, 14 + press * 9, 3 + press * 1.2, -pivotDir * 0.055, Math.PI * 0.05, Math.PI * 1.8);
    ctx.stroke();
    if (pivot.stopRaw > 0.03 || pivot.moveRaw > 0.03) {
      ctx.strokeStyle = `rgba(255, 238, 184, ${0.035 + pivot.activeAmount * 0.05})`;
      ctx.lineWidth = 0.9 + pivot.activeAmount * 0.7;
      ctx.beginPath();
      ctx.moveTo(pivotX - pivotDir * 4, FLOOR + 6);
      ctx.quadraticCurveTo(
        pivotX - pivotDir * (18 + press * 10),
        FLOOR + 11,
        pivotX - pivotDir * (34 + press * 16 + snap * 8),
        FLOOR + 7
      );
      ctx.stroke();
    }
...
function drawMovementPoseFX(f, crouch, walking, stride) {
  ...
  if (jump <= 0.03 && aerialTuck <= 0.03 && preLand <= 0.03 && landing <= 0.03 && facingTurn <= 0.03 && pivot.activeAmount <= 0.03 && startPresence.active <= 0.03 && air <= 0.03 && plant <= 0.08 && footPlant <= 0.03 && push <= 0.03 && stopStrength <= 0.03 && movePresent.active <= 0.03 && guardPresent.active <= 0.03 && attackPlant <= 0.03 && attackCompression <= 0.03 && reactionSkid <= 0.03) return;
  ...
  if (guardPresent.active > 0.04 && f.grounded && !f.attack && f.hurt <= 0) {
    const press = clamp(Math.max(guardPresent.footPress, guardPresent.brace * 0.55), 0, 1.45);
    const snap = clamp(guardPresent.counterSnap + guardPresent.impact * 0.42, 0, 1.35);
    const guardColor = guardPresent.guardProfile.color ?? trim;
    ctx.strokeStyle = colorWithAlpha(guardColor, 0.04 + press * 0.055 + snap * 0.035);
    ctx.lineWidth = 0.85 + press * 0.7 + snap * 0.3;
    for (const side of [-1, 1]) {
      const footX = side * (30 + guardPresent.footSpread * 7);
      ctx.beginPath();
      ctx.ellipse(footX, 5 + localCrouch + press * 0.45, 17 + press * 8 + snap * 3, 3.4 + press * 1.2, side * 0.035, 0, Math.PI * 1.86);
      ctx.stroke();
    }

    if (guardPresent.impact > 0.05 || guardPresent.counter > 0.05) {
      ctx.strokeStyle = colorWithAlpha(trim, 0.045 + guardPresent.impact * 0.07 + guardPresent.counter * 0.055);
      ctx.lineWidth = 0.8 + guardPresent.impact * 0.75 + guardPresent.counter * 0.45;
      ctx.beginPath();
      ctx.moveTo(-35, -123 + localCrouch + guardPresent.brace * 2);
      ctx.quadraticCurveTo(-4, -139 + localCrouch - snap * 6, 36 + snap * 5, -119 + localCrouch + guardPresent.recoil * 3);
      ctx.stroke();
    }
  }
  ...
  if (facingTurn > 0.03) {
    ...
    ctx.beginPath();
    ctx.ellipse(pivotSide * 30, 3 + localCrouch, 18 + facingPress * 8, 4.2 + facingPress * 1.4, -turnDir * 0.06, 0, Math.PI * 2);
    ctx.fill();
  }

  if (pivot.activeAmount > 0.04) {
    const pivotDir = pivot.faceRaw > 0.03 ? pivot.faceDir : pivot.stopRaw > 0.03 ? pivot.stopDir : pivot.moveDir;
    ...
    if (pivot.stopRaw > 0.03 || pivot.moveRaw > 0.03) {
      ctx.strokeStyle = colorWithAlpha(trim, 0.045 + drag * 0.055);
      ctx.lineWidth = 0.95 + drag * 0.7;
      ctx.beginPath();
      ctx.moveTo(support - pivotDir * 4, 5 + localCrouch);
      ctx.quadraticCurveTo(support - pivotDir * (17 + drag * 8), 9 + localCrouch, support - pivotDir * (33 + drag * 15), 6 + localCrouch);
      ctx.stroke();
    }

    if (pivot.snap > 0.05) {
      ctx.strokeStyle = `rgba(255, 247, 214, ${0.038 + pivot.snap * 0.045})`;
      ctx.lineWidth = 0.8 + pivot.snap * 0.55;
      ctx.beginPath();
...
function drawFighterIdentityAura(f, crouch, walking = false, stride = 0) {
  ...
  const idleLife = idle ? idleVisualMotion(f) : null;
  if (idleLife && idleLife.active > 0.03) {
    ctx.save();
    ctx.globalCompositeOperation = "screen";
    for (const side of [-1, 1]) {
      const press = side > 0 ? idleLife.frontPress : idleLife.backPress;
      const footX = side * (30 + (idleLife.style.wide - 1) * 7) * scale + idleLife.sway * 1.8;
      ctx.strokeStyle = colorWithAlpha(trim, 0.035 + press * 0.04);
      ctx.lineWidth = 0.8 + press * 0.45;
      ctx.beginPath();
      ctx.ellipse(footX, 4.5 + localCrouch + press * 0.4, (15 + press * 5) * scale, (2.7 + press * 0.9) * scale, side * 0.035, 0, Math.PI * 2);
      ctx.stroke();
    }
    ctx.restore();
  }
...
function drawUnifiedSpriteAnatomyPolish(f, crouch, frameName, frameIndex, walking, stride) {
  ...
  drawUnifiedPivotMotionPolish(f, localCrouch, walking, stride);
  drawUnifiedIdleLifePolish(f, localCrouch, walking, stride);
...
function drawUnifiedPivotMotionPolish(f, localCrouch, walking, stride) {
  ...
  const footY = -2 + localCrouch;
  const shoulderY = heavy ? -145 : -150;
  const chestY = heavy ? -120 : -122;
  const hipY = heavy ? -62 : -59;
  ...
  ctx.save();
  ctx.globalCompositeOperation = "multiply";
  ctx.fillStyle = `rgba(18, 9, 7, ${0.032 + load * 0.062})`;
  ctx.beginPath();
  ctx.ellipse(
    counter * 0.28,
...
function drawUnifiedGuardCommitmentPolish(f, localCrouch) {
  if (!f.grounded || f.hurt > 0 || f.attack || winner) return;

  const defense = guardPresentationMotion(f);
  const guard = defense.guard;
  const impact = defense.impact;
  const counter = defense.counter;
  const active = clamp(Math.max(guard * 0.48, impact, counter * 0.72, defense.entry * 0.45), 0, 1.4);
  if (active <= 0.035) return;

  const guardProfile = defense.guardProfile;
  const commitment = defense.commitment;
  const trim = f.trim ?? "#fff1bd";
  const guardColor = guardProfile.color ?? trim;
  const heavy = f.profileId === "p1";
  const pulse = 0.5 + Math.sin(roundFrame * 0.34 + (heavy ? 0.2 : 0.9)) * 0.5;
  const snap = defense.counterSnap * commitment.snap;
  const shoulderY = heavy ? -145 : -150;
  const chestY = heavy ? -119 : -121;
  ...
  if (counter > 0.045) {
    ctx.strokeStyle = `rgba(255, 241, 189, ${0.065 + counter * 0.13 + pulse * 0.025})`;
    ctx.lineWidth = 1 + counter * 0.9;
    ctx.beginPath();
    ctx.moveTo(-30, chestY + localCrouch - 9 - snap * 1.2);
    ctx.quadraticCurveTo(0, chestY + localCrouch - 20 - snap * 8, 32 + snap * 4, chestY + localCrouch - 7);
    ctx.moveTo(-28, waistY + localCrouch + 5);
    ctx.quadraticCurveTo(2, waistY + localCrouch + 13 + snap * 2, 34 + snap * 5, waistY + localCrouch + 2);
    ctx.stroke();
  }
...
function drawUnifiedImpactRecoveryPolish(f, localCrouch, motion, acting) {
  ...
  if (damage > 0.045) {
    ...
    drawReactionCompressionEcho(f, localCrouch);
  }
...
function drawReactionCompressionEcho(f, localCrouch, reaction = reactionPresentationProfile(f), accent = null) {
  if (reaction.active <= 0.04 || reaction.kind === "guard") return;
  const force = clamp(reaction.force, 0, 1.55);
  const crease = clamp(reaction.creaseAlpha, 0, 0.9);
  const shadow = clamp(reaction.shadowWeight, 0, 1.2);
...
function drawUnifiedIdleLifePolish(f, localCrouch, walking, stride) {
  const idle = idleVisualMotion(f);
  if (idle.active <= 0.03 || walking || Math.abs(stride ?? 0) > 0.12) return;

  const heavy = f.profileId === "p1";
  const trim = f.trim ?? "#fff1bd";
  const breath = idle.breath * idle.style.breath;
  const sway = idle.sway;
  const hand = idle.ready * idle.style.hand;
  const hip = (idle.frontPress - idle.backPress) * idle.style.hip;
  const shoulderY = heavy ? -145 : -150;
  const chestY = heavy ? -118 : -121;
  const hipY = heavy ? -61 : -58;
  const footY = -2 + localCrouch;

  ctx.save();
  ctx.globalCompositeOperation = "multiply";
  ctx.fillStyle = `rgba(18, 9, 7, ${0.03 + Math.abs(breath) * 0.026 + Math.abs(hip) * 0.018})`;
  ctx.beginPath();
  ctx.ellipse(
    sway * 1.2,
    shoulderY + localCrouch - Math.max(0, breath) * 1.1,
    (heavy ? 31 : 25) + Math.max(0, breath) * 2.5,
    (heavy ? 8.8 : 7.4) + Math.max(0, breath) * 1.1,
    sway * 0.018,
    0,
    Math.PI * 2
  );
  ctx.ellipse(
    hip * 1.8,
    hipY + localCrouch + Math.max(0, -breath) * 1.4,
    (heavy ? 45 : 36) + Math.abs(hip) * 4,
    (heavy ? 11 : 9.5) + Math.max(idle.frontPress, idle.backPress) * 1.8,
    hip * 0.015,
    0,
    Math.PI * 2
  );
  ...
  ctx.fillStyle = `rgba(20, 11, 7, ${0.04 + Math.max(idle.frontPress, idle.backPress) * 0.055})`;
  for (const side of [-1, 1]) {
    const press = side > 0 ? idle.frontPress : idle.backPress;
    ctx.beginPath();
    ctx.ellipse(
      side * (heavy ? 34 : 29) + hip * 2.2,
      footY + 5 + press * 0.5,
      (heavy ? 24 : 20) + press * 8,
      4.7 + press * 1.2,
      side * 0.035,
  ...
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  ctx.strokeStyle = colorWithAlpha(trim, 0.045 + Math.abs(breath) * 0.045 + hand * 0.035);
  ctx.lineWidth = 0.9 + hand * 0.42;
  ctx.beginPath();
  ctx.moveTo(-28 + sway * 0.6, chestY + localCrouch - Math.max(0, breath) * 1.4);
  ctx.quadraticCurveTo(-9 + hip * 0.8, chestY + 13 + localCrouch + breath * 1.2, -2, chestY + 10 + localCrouch);
  ctx.moveTo(28 + sway * 0.6, chestY + localCrouch - Math.max(0, breath) * 1.1);
  ctx.quadraticCurveTo(9 + hip * 0.8, chestY + 14 + localCrouch - breath * 0.8, 2, chestY + 11 + localCrouch);
  ctx.stroke();

  ctx.strokeStyle = colorWithAlpha(trim, 0.05 + hand * 0.08);
  ctx.lineWidth = 0.8 + hand * 0.45;
  for (const side of [-1, 1]) {
    const x = side * (heavy ? 47 : 42) + sway * 1.2;
    const y = (heavy ? -107 : -112) + localCrouch - hand * (side > 0 ? 1.8 : 1.2);
    ctx.beginPath();
    ctx.moveTo(x - side * 6, y);
    ctx.quadraticCurveTo(x, y - 3.5 - hand * 2.2, x + side * (7 + hand * 2.4), y - 0.6);
    ctx.stroke();
  }

  ctx.strokeStyle = `rgba(255, 229, 154, ${0.045 + Math.max(idle.frontPress, idle.backPress) * 0.045})`;
  ctx.lineWidth = 0.9 + Math.max(idle.frontPress, idle.backPress) * 0.35;
  for (const side of [-1, 1]) {
    const press = side > 0 ? idle.frontPress : idle.backPress;
    ctx.beginPath();
    ctx.ellipse(side * (heavy ? 34 : 29) + hip * 2.2, footY + 2, 16 + press * 5, 3.2 + press * 0.6, side * 0.035, 0, Math.PI * 2);
    ctx.stroke();
  }
  ctx.restore();
}
...
function drawVectorContactOcclusion(f, crouch, stride) {
  ...
  ctx.save();
  ctx.globalCompositeOperation = "multiply";
  ctx.fillStyle = `rgba(24, 12, 9, ${0.16 + action * 0.04 + guard * 0.04 + defense.brace * 0.018 + hurt * 0.04})`;
  ctx.beginPath();
  ...
  ctx.strokeStyle = `rgba(18, 9, 7, ${0.08 + action * 0.08 + guard * 0.1 + defense.handGuard * 0.025 + defense.impact * 0.035})`;
  ctx.lineWidth = 5.8 + guard * 1.6 + defense.handGuard * 0.6;
  ctx.lineCap = "round";
  ctx.beginPath();
  ...
  ctx.quadraticCurveTo(18, -105 + crouch, 9, -82 + crouch + breath);
  ctx.stroke();
  if (defense.active > 0.04 && f.grounded && f.hurt <= 0 && !f.attack && !winner) {
    ctx.strokeStyle = `rgba(18, 9, 7, ${0.045 + defense.brace * 0.05 + defense.impact * 0.045})`;
    ctx.lineWidth = 4.2 + defense.brace * 1.4;
    ctx.beginPath();
    ctx.moveTo(-shoulder + 11, -118 + crouch + defense.shoulderSink * 1.5);
    ctx.quadraticCurveTo(-8 - defense.handImpact * 3, -113 + crouch + defense.brace * 4, 18 + defense.counterSnap * 4, -104 + crouch + defense.crouchSettle * 3);
    ctx.moveTo(shoulder - 10, -118 + crouch + defense.shoulderSink * 1.2);
    ctx.quadraticCurveTo(13 + defense.handImpact * 2, -113 + crouch + defense.brace * 4, -6 + defense.counterSnap * 3, -104 + crouch + defense.crouchSettle * 3);
    ctx.stroke();
  }
  ctx.restore();
...
function drawSpriteExtremityDetails(f, crouch, frameName, stride, options = {}) {
  ...
  applyGuardExtremityPresence(hands, feet, f, frameName);
  ...
  applyReactionExtremityPresence(hands, feet, f, frameName);
  if (!options.unified) {
    applyWhiffFootPressure(feet, f);
    applyWhiffExtremityPresence(hands, feet, f);
  }
...
function spriteHandDetailPose(frameName, action, guard, hurt, localCrouch, f = null) {
...
function applyIdleFootPressure(feet, f, frameName) {
  if (frameName !== "idle" || !Array.isArray(feet) || feet.length < 2) return;
  const idle = idleVisualMotion(f);
  if (idle.active <= 0.03) return;

  const style = idle.style;
  const left = feet[0];
  const right = feet[1];
...
function applyGuardExtremityPresence(hands, feet, f, frameName) {
  if (!Array.isArray(hands) || !Array.isArray(feet) || hands.length < 2 || feet.length < 2) return;
  if (!f?.grounded || f.attack || f.hurt > 0 || winner) return;
  if (frameName !== "block" && frameName !== "idle") return;
...
function applyAttackExtremityPresence(hands, feet, f, frameName) {
  ...
  if (spec.punch || spec.special || spec.grab) {
    ...
    frontHand.sy -= handGrip * 0.025;
    frontHand.angle -= attack.reach * (spec.special ? 0.035 : 0.052);
    frontHand.curl = clamp((frontHand.curl ?? 0.76) + handGrip * (spec.grab ? 0.18 : 0.12), 0, 1.16);
    frontHand.shock = Math.max(frontHand.shock ?? 0, finger * 0.72);

    backHand.x -= attack.load * 2.2 + attack.follow * 1.4;
    backHand.y -= attack.load * 1.8 - attack.recover * 1.1;
    ...
    frontHand.curl = clamp((frontHand.curl ?? 0.72) + handGrip * 0.08, 0, 1.08);
    frontHand.shock = Math.max(frontHand.shock ?? 0, finger * 0.35);

    backHand.x -= attack.reach * (spec.sweep ? 3.4 : 4.4);
    backHand.y += attack.drive * (spec.sweep ? 2.4 : 1.3);
    backHand.angle += attack.drive * 0.04;
    backHand.curl = clamp((backHand.curl ?? 0.72) + handGrip * 0.07, 0, 1.05);
    backHand.shock = Math.max(backHand.shock ?? 0, finger * 0.32);
  }

  const supportIndex = attack.supportSide < 0 ? 0 : 1;
  const freeIndex = supportIndex === 0 ? 1 : 0;
  const support = feet[supportIndex];
  const free = feet[freeIndex];
...
function applyPivotFootPressure(feet, f) {
  ...
  support.press = Math.max(support.press ?? 0, press);
  support.plant = Math.max(support.plant ?? 0, 0.68 + press * 0.26);
  support.w += press * 5.4 + snap * 1.6;
  support.h = Math.max(7.1, support.h - press * 1.05);
  support.y += press * 1.2;
  support.x -= localDir * (press * 2.1 + snap * 0.8);
  support.angle += supportIndex === 0 ? -press * 0.042 + snap * 0.012 : press * 0.042 - snap * 0.012;

  if (release) {
    release.plant = Math.max(0.16, (release.plant ?? 0.72) - snap * 0.16 - press * 0.06);
    release.press = Math.max(release.press ?? 0, snap * 0.11);
    release.y -= snap * 1.25;
...
function applyReactionExtremityPresence(hands, feet, f, frameName) {
  if (!f?.grounded || f.hurt <= 0 || !Array.isArray(hands) || !Array.isArray(feet) || hands.length < 2 || feet.length < 2) return;
  if (frameName !== "hurt" && frameName !== "defeat") return;
  const reaction = reactionPresentationProfile(f);
  if (reaction.active <= 0.03 || reaction.kind === "guard") return;

  const headHit = reaction.zone === "head";
...
function applyWhiffExtremityPresence(hands, feet, f) {
  if (!Array.isArray(hands) || !Array.isArray(feet) || hands.length < 2 || feet.length < 2) return;
  const whiff = whiffRecoveryMotion(f);
  if (!whiff) return;
  ...
  for (const foot of feet) {
    foot.toeFlex = Math.max(foot.toeFlex ?? 0, toeDrag * (foot.plant > 0.4 ? 0.9 : 0.45));
    foot.sole = Math.max(foot.sole ?? 1, 1 + toeDrag * 0.08);
  }
}

function applyFacingTurnFootPressure(feet, f) {
  if (!f?.grounded || f.hurt > 0 || f.attack || !Array.isArray(feet) || feet.length < 2) return;
...
function drawSpriteHandDetail(hand, skin, trim) {
  const shock = clamp(hand.shock ?? 0, 0, 1.15);
  ctx.save();
  ...
  ctx.globalCompositeOperation = "multiply";
  ctx.fillStyle = `rgba(32, 16, 12, ${0.16 + shock * 0.07})`;
  ctx.beginPath();
  ctx.ellipse(1 - shock * 0.6, 3 + shock * 0.5, 14 + shock * 1.8, 7 - shock * 0.45, 0.1, 0, Math.PI * 2);
  ctx.fill();

  ctx.strokeStyle = "rgba(47, 25, 18, 0.34)";
  ctx.lineWidth = 1.25 + shock * 0.3;
  ctx.lineCap = "round";
  for (let i = -1; i <= 1; i += 1) {
    const x = i * 4.2;
    ctx.beginPath();
    ctx.moveTo(x - 1.2 - shock * 0.25, -5.4 - shock * 0.4);
    ctx.quadraticCurveTo(x + hand.curl * 0.8 + shock * 0.7, -0.5 + shock * 0.35, x + 1.1 + shock * 0.6, 4.2 - shock * 0.25);
    ctx.stroke();
  }

  ctx.strokeStyle = "rgba(35, 18, 13, 0.44)";
  ctx.lineWidth = 1.4 + shock * 0.25;
  ctx.beginPath();
  ctx.moveTo(-8.5, -2.5);
  ctx.quadraticCurveTo(-2, -7.4, 8.5, -2.6);
  ctx.stroke();
  if (shock > 0.08) {
    ctx.strokeStyle = `rgba(24, 12, 9, ${0.12 + shock * 0.16})`;
    ctx.lineWidth = 0.9;
    ctx.beginPath();
    ctx.moveTo(-6.5, 2.4 + shock * 0.8);
    ctx.quadraticCurveTo(-1, 5.2 + shock * 0.7, 6.2, 2.8);
    ctx.stroke();
  }
  ctx.restore();
  ...
  ctx.scale(hand.sx, hand.sy);
  ctx.globalCompositeOperation = "screen";
  ctx.strokeStyle = colorWithAlpha(lighten(skin, 26), 0.18 + shock * 0.08);
  ctx.lineWidth = 1.2 + shock * 0.18;
  ctx.beginPath();
  ctx.moveTo(-7, -5.8);
  ctx.quadraticCurveTo(0, -9.6, 8.2, -4.5);
  ctx.stroke();
  ctx.strokeStyle = colorWithAlpha(trim, 0.09 + shock * 0.06);
  ctx.lineWidth = 1.4;
  ctx.beginPath();
...
function drawSpriteFootDetail(foot, shoe, trim) {
  ...
  ctx.lineWidth = 1.7 + press * 0.7 + toeFlex * 0.28;
  ctx.lineCap = "round";
  ctx.beginPath();
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    ctx.quadraticCurveTo(x + 1.1 + toeFlex * 0.5, foot.h * 0.02 + toeFlex * 0.6, x + 1.8 + toeFlex * 1.1, foot.h * 0.24 - toeFlex * 0.2);
    ctx.stroke();
  }
  ctx.restore();
...
function drawSpriteGuardPoseOverlay(f, crouch, frameName) {
  ...
  const impact = defense.impact;
  const counter = defense.counter;
  const active = Math.max(guard, impact, counter * 0.84, defense.entry * 0.6);
  ...
  ctx.globalCompositeOperation = "screen";
  ctx.globalAlpha = clamp((0.08 + guard * 0.13 + counterPulse * 0.2 + impact * 0.2 + defense.shield * 0.035) * guardProfile.shieldAlpha, 0.05, 0.42);
...
function drawSpriteAttackMotionTrace(f, crouch, type, power, read, phase) {
  ...
  const presentReach = clamp(presentation.reach, 0, 1.4);
  const presentCrease = clamp(presentation.creaseAlpha, 0, 0.8);
  ...
  const endX = (sweep ? 158 + power * 28 : kick ? 164 + power * 34 : special ? 144 + power * 20 : 152 + power * 26) + presentReach * (kick || sweep ? 9 : 6);
  const arcY = (sweep ? -24 + localCrouch : kick ? -105 + localCrouch - snap * 8 : special ? -154 + localCrouch - snap * 10 : -152 + localCrouch - snap * 7) - presentReach * (kick || special ? 3.4 : 2.2);
  ...
  if (presentCrease > 0.04) {
    ctx.globalAlpha = clamp(0.018 + presentCrease * 0.052, 0.015, 0.07);
    ctx.strokeStyle = colorWithAlpha(special ? f.specialStyle?.rim ?? trim : "#ffffff", 0.78);
    ctx.lineWidth = 0.65 + snap * 0.34;
    ctx.beginPath();
    ctx.moveTo(endX - 42 - presentReach * 6, baseY - (kick ? 12 : 7));
    ctx.quadraticCurveTo(endX - 18, arcY - presentReach * 4, endX + 18 + presentReach * 4, baseY - (sweep ? 2 : 8));
    ctx.stroke();
  }
...
function drawAttackPresentationEcho(f, crouch, attack = attackPresentationProfile(f)) {
  if (!attack || attack.active <= 0.045) return;
  const spec = attack.visualSpec;
  const localCrouch = crouch * 0.18;
  const trim = f.trim ?? "#fff1bd";
  const force = clamp(attack.drive * 0.48 + attack.reach * 0.34 + attack.follow * 0.18, 0, 1.4);
  const shadow = clamp(attack.plantShadow, 0, 1.18);
  const crease = clamp(attack.creaseAlpha, 0, 0.86);
  const supportX = attack.supportSide * -34;

  if (f.grounded && shadow > 0.035) {
    ctx.save();
    ctx.globalCompositeOperation = "multiply";
    ctx.fillStyle = `rgba(22, 12, 8, ${clamp(0.032 + shadow * 0.064, 0.025, 0.11)})`;
    ctx.beginPath();
    ctx.ellipse(supportX - attack.supportSide * force * 5, -3 + localCrouch + shadow * 0.7, 30 + shadow * 14, 4.8 + shadow * 2.2, -attack.supportSide * 0.045, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
  ...
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  const tipX = attack.tipX;
  const tipY = attack.tipY + localCrouch;
  const height = spec.sweep ? 9 : spec.kick ? 12 : spec.special ? 18 : 11;
  const reach = spec.sweep || spec.kick ? 36 : spec.special ? 30 : 26;
  ctx.strokeStyle = colorWithAlpha(spec.special ? f.specialStyle?.rim ?? trim : trim, clamp(0.045 + crease * 0.095, 0.04, 0.13));
  ctx.lineWidth = 0.85 + force * 0.58;
  ctx.beginPath();
  ctx.moveTo(tipX - reach * 0.82, tipY - height * 0.55);
  ctx.quadraticCurveTo(tipX - reach * 0.18, tipY - height * (1.05 + attack.reach * 0.18), tipX + reach * 0.58, tipY - height * 0.2);
  ctx.moveTo(tipX - reach * 0.62, tipY + height * 0.38);
  ctx.quadraticCurveTo(tipX - reach * 0.14, tipY + height * 0.72 + attack.follow * 2.4, tipX + reach * 0.46, tipY + height * 0.22);
  ctx.stroke();

  if (attack.fingerTension > 0.08 || attack.toeDig > 0.08) {
    ctx.globalAlpha = clamp(0.032 + Math.max(attack.fingerTension, attack.toeDig) * 0.06, 0.025, 0.1);
    ctx.strokeStyle = colorWithAlpha("#ffffff", 0.78);
    ctx.lineWidth = 0.75 + force * 0.25;
    ctx.beginPath();
    ctx.moveTo(tipX - reach * 0.15, tipY - height * 0.34);
    ctx.lineTo(tipX + reach * (spec.kick || spec.sweep ? 0.62 : 0.42), tipY - height * 0.2);
    ctx.stroke();
  }
  ctx.restore();
}
...
function drawAttackBodyGlow(f, crouch) {
  ...
  ctx.globalAlpha = 1;
  drawAttackPresentationEcho(f, crouch, presentation);
  ctx.globalAlpha = 1;
  ctx.restore();
}
...
function drawDamageReactionFX(f, crouch) {
  ...
  if (damage > 0.03) {
    ...
    drawReactionCompressionEcho(f, crouch, reactionPresentationProfile(f), flavor.highlight);
  }
...
function drawEntrancePoseEffect(f, crouch) {
  ...
  if (f.profileId === "p2") {
    ...
    ctx.strokeStyle = colorWithAlpha(trim, 0.1 + alpha * 0.28);
    ctx.lineWidth = 1.4 + profile.flourish * 0.7;
    ctx.beginPath();
    const arc = profile.lift > 1 ? -1 : 1;
    ctx.moveTo(-34 * profile.foot, -34 + crouch);
    ctx.quadraticCurveTo(arc * 12 * profile.flourish, -102 + crouch - entrance.step * 8, 38 * profile.foot, -72 + crouch);
    ctx.stroke();
  }
  ctx.restore();
}
...
function drawAttackArc(f, box) {
  ...
  if (spec.grab) {
    ...
    const recoil = phase.recovery * 0.72;
    ctx.save();
    ctx.globalCompositeOperation = "screen";
    ctx.translate(x, y);
    ...
    ctx.lineCap = "round";
    ctx.strokeStyle = colorWithAlpha(spec.color, 0.16 + catchT * 0.26);
    ctx.lineWidth = 2.4 + open * 2.4;
    for (const side of [-1, 1]) {
      ctx.beginPath();
      ctx.moveTo(-12 - phase.anticipation * 9, side * (14 + phase.anticipation * 10));
      ctx.quadraticCurveTo(
        34 + open * 20,
    ...
    ctx.strokeStyle = colorWithAlpha("#ffffff", 0.12 + phase.snap * 0.2);
    ctx.lineWidth = 1.2 + phase.snap * 1.1;
    ctx.beginPath();
    ctx.ellipse(reach + catchT * 18, 0, 18 + catchT * 12, 28 + catchT * 10, 0, Math.PI * 0.16, Math.PI * 1.84);
    ctx.stroke();

    if (phase.snap > 0.05) {
      ctx.fillStyle = colorWithAlpha(spec.core, 0.14 + phase.snap * 0.2);
      for (const side of [-1, 1]) {
        ctx.beginPath();
        ctx.ellipse(reach + 12 + phase.snap * 12, side * 12, 10 + phase.snap * 8, 5 + phase.snap * 3, side * 0.12, 0, Math.PI * 2);
        ctx.fill();
      }
    }
    ctx.restore();
...
function drawWhiffRecoveryFX(f, crouch, baseX = f.x, baseY = f.y) {
  ...
  ctx.fillStyle = glow;
  ctx.beginPath();
  ctx.ellipse(x + localDir * 28, y + crouch * 0.42, trailReach * 0.58, height + fade * 9 * whiffStyle.arm, localDir * -0.07, 0, Math.PI * 2);
  ctx.fill();
  ...
  if (rebound > 0.04) {
    ...
    ctx.lineWidth = 0.75 + rebound * 0.9;
    for (let i = 0; i < 2; i += 1) {
      const lane = i === 0 ? -0.36 : 0.42;
      ctx.beginPath();
      ctx.moveTo(x + localDir * trailReach * (0.28 + i * 0.08), y + crouch * 0.42 + height * lane);
...
function attackPresentationProfile(f) {
  const attack = f?.attack;
  const phase = attackPhase(attack);
  const mass = attackMassProfile(f);
  const body = attackBodyWeightProfile(f);
  const read = attackReadabilityProfile(f);
  const visualSpec = read.spec ?? attackVisualSpec(attack?.type);
  const type = attack?.type ?? "";
  const active = clamp(Math.max(read.active, mass.active * 0.82, body.active * 0.58, phase.followThrough * 0.42), 0, 1.55);
  const supportSide = body.footSide < 0 ? -1 : 1;
  const freeSide = -supportSide;
  const style = {
    p1: { reach: 0.9, hand: 0.86, foot: 1.18, crease: 0.92, shadow: 1.18, snap: 0.82 },
    p2: { reach: 1.16, hand: 1.2, foot: 0.84, crease: 1.14, shadow: 0.84, snap: 1.2 },
    p3: { reach: 0.82, hand: 0.76, foot: 1.3, crease: 0.82, shadow: 1.3, snap: 0.72 },
    p4: { reach: 1.12, hand: 1.16, foot: 0.88, crease: 1.16, shadow: 0.88, snap: 1.16 },
    p5: { reach: 1.24, hand: 1.26, foot: 0.78, crease: 1.22, shadow: 0.78, snap: 1.24 },
    p6: { reach: 0.96, hand: 0.94, foot: 1.06, crease: 0.98, shadow: 1.06, snap: 0.94 },
  }[f?.profileId] ?? { reach: 1, hand: 1, foot: 1, crease: 1, shadow: 1, snap: 1 };

  if (!attack || active <= 0.012 || (f?.hurt ?? 0) > 0 || winner) {
    return {
      active: 0,
      type,
      visualSpec,
  ...
  const drive = clamp(mass.drive + phase.snap * 0.48, 0, 1.85);
  const load = clamp(mass.load + phase.anticipation * 0.28, 0, 1.75);
  const follow = clamp(mass.follow + phase.followThrough * 0.34, 0, 1.6);
  const recover = clamp(mass.recover + phase.recovery * 0.2, 0, 1.6);
  const limbWeight = visualSpec.kick || visualSpec.sweep ? 1.16 : visualSpec.special ? 1.08 : visualSpec.grab ? 1.02 : 0.94;
  const reach = clamp((drive * 0.6 + read.clarity * 0.32 + follow * 0.14) * style.reach * limbWeight, 0, 1.65);
  const handTension = clamp((load * 0.22 + drive * 0.42 + follow * 0.24 + read.wind * 0.18) * style.hand, 0, 1.2);
  const fingerTension = clamp((phase.snap * 0.38 + read.clarity * 0.32 + follow * 0.18) * style.hand * style.snap, 0, 1.12);
  const toeDig = clamp((body.plant * 0.48 + body.brace * 0.28 + drive * 0.18 + recover * 0.12) * style.foot, 0, 1.24);
  const plantShadow = clamp((body.plant * 0.58 + body.compression * 0.26 + load * 0.12) * style.shadow, 0, 1.22);
  const creaseAlpha = clamp((active * 0.24 + read.clarity * 0.18 + phase.snap * 0.12) * style.crease, 0, 0.86);
  const tipX = visualSpec.sweep ? 120 + reach * 32 : visualSpec.kick ? 116 + reach * 28 : visualSpec.special ? 94 + reach * 22 : visualSpec.grab ? 86 + reach * 20 : 96 + reach * 24;
  const tipY = visualSpec.sweep ? -18 : visualSpec.kick ? -70 - reach * 7 : visualSpec.special ? -124 - reach * 8 : visualSpec.grab ? -111 - reach * 3 : -128 - reach * 5;
...
function getPose(f, stride) {
  ...
  if (f.blocking) {
    ...
    const guardHold = guardPresent.guard;
    const guardEntry = guardPresent.entry;
    const guardImpact = guardPresent.impact;
    const guardBreath = Math.sin(roundFrame * 0.11 + (f.profileId?.charCodeAt(1) ?? 0)) * guardHold;
    const guardSnap = Math.max(guardEntry, guardPresent.recoil, guardPresent.counterSnap * 0.42);
    ...
    const guardLow = Math.max(smoothStep01(clamp(f.crouch ?? 0, 0, 1)), guardPresent.crouchSettle * 0.72);
    const entrySnap = guardEntry;
  ...
  const guardImpactBody = guardPresent.impact;
  const guardHoldBody = guardPresent.guard;
  const guardEntryBody = guardPresent.entry;
  const counterBody = !winner && f.hurt <= 0 && !f.attack ? guardPresent.counter : 0;
  const guardCommitBody = Math.max(guardImpactBody, guardEntryBody * 0.72, guardHoldBody * 0.38, counterBody * 0.7, guardPresent.footPress * 0.35);
  if (guardCommitBody > 0.025 && f.grounded && f.hurt <= 0 && !f.attack && !winner) {
    const commitment = guardPresent.commitment;
    const pulse = Math.sin(roundFrame * 0.22 + (f.profileId?.charCodeAt(1) ?? 0)) * guardCommitBody;
  ...
  if (attackType === "punch" || attackType === "airPunch") {
    ...
    const grabStyle = {
      ...
      p3: { brace: 1.32, reach: 0.84, spread: 1.18, snap: 0.66 },
      p4: { brace: 0.86, reach: 1.14, spread: 0.9, snap: 1.14 },
      p5: { brace: 0.76, reach: 1.26, spread: 0.82, snap: 1.24 },
      p6: { brace: 1.08, reach: 0.96, spread: 1.02, snap: 0.9 },
    }[f.profileId] ?? { brace: 1, reach: 1, spread: 1, snap: 1 };

    base.frontArm.elbow = {
      x: (43 - windup * 16 + reach * 48 - drag * 12) * spec.stance * grabStyle.reach,
      y: -101 + crouch - windup * 7 - catchSnap * 12 + drag * 9,
    };
    base.frontArm.hand = {
      x: (62 - windup * 24 + reach * 84 - drag * 18) * spec.stance * grabStyle.reach,
      y: -96 + crouch - windup * 9 - catchSnap * 15 + drag * 12,
    };
    base.backArm.elbow = {
      x: (13 - windup * 14 + reach * 50 - drag * 10) * spec.stance * grabStyle.reach,
      y: -118 + crouch - windup * 8 - catchSnap * 7 + drag * 8,
    };
    base.backArm.hand = {
    ...
    if (f.profileId === "p2" || f.profileId === "p4" || f.profileId === "p5") {
      ...
      base.frontLeg.foot.y -= catchSnap * 1.6;
    } else if (f.profileId === "p1" || f.profileId === "p3") {
      base.backLeg.knee.y += load * 4;
      base.backLeg.foot.x -= load * 5 * spec.stance;
      base.frontArm.hand.y += reach * 3;
      base.backArm.hand.y += reach * 2;
    }
  ...
  if (whiffPose) {
    ...
    if (looseFingers > 0.025) {
      ...
      base.frontArm.fingerFidget = Math.max(base.frontArm.fingerFidget ?? 0, looseFingers);
      base.backArm.fingerFidget = Math.max(base.backArm.fingerFidget ?? 0, looseFingers * 0.72);
      base.frontArm.hand.y += looseFingers * (leg ? 1.8 : 1.1);
      base.backArm.hand.y += looseFingers * (leg ? 2.1 : 1.2);
    }
    if (toeRecover > 0.025) {
      base.backLeg.toeFlex = Math.max(base.backLeg.toeFlex ?? 0, toeRecover);
      base.frontLeg.toeFlex = Math.max(base.frontLeg.toeFlex ?? 0, toeRecover * (leg ? 0.75 : 0.42));
  ...
  if (f.attack && attackPresent.active > 0.035) {
    ...
    if (specPresent.punch || specPresent.special || specPresent.grab) {
      ...
      base.backArm.handCurl = clamp((base.backArm.handCurl ?? 0.5) + handTension * 0.08, 0, 1);
      base.frontArm.fingerFidget = Math.max(base.frontArm.fingerFidget ?? 0, finger);
      base.backArm.fingerFidget = Math.max(base.backArm.fingerFidget ?? 0, finger * 0.54);
      base.frontArm.hand.x += attackPresent.reach * (specPresent.special ? 2.4 : specPresent.grab ? 2.8 : 3.2) * spec.stance;
      base.frontArm.hand.y -= attackPresent.reach * (specPresent.special ? 1.7 : 1.1);
      base.backArm.hand.x -= attackPresent.load * 1.4 * spec.stance;
    } else {
      base.frontArm.handCurl = clamp((base.frontArm.handCurl ?? 0.5) + handTension * 0.08, 0, 1);
      base.backArm.handCurl = clamp((base.backArm.handCurl ?? 0.5) + handTension * 0.07, 0, 1);
      base.frontArm.fingerFidget = Math.max(base.frontArm.fingerFidget ?? 0, finger * 0.4);
      base.backArm.fingerFidget = Math.max(base.backArm.fingerFidget ?? 0, finger * 0.36);
      base.frontLeg.footAngle = (base.frontLeg.footAngle ?? 0) - attackPresent.reach * (specPresent.sweep ? 0.018 : 0.032);
      base.frontLeg.toeFlex = Math.max(base.frontLeg.toeFlex ?? 0, toe * (specPresent.sweep ? 0.68 : 0.9));
    }

    base[supportKey].plant = Math.max(base[supportKey].plant ?? 0, clamp(0.56 + attackPresent.plantShadow * 0.28, 0, 1));
    base[supportKey].toeFlex = Math.max(base[supportKey].toeFlex ?? 0, toe);
    base[supportKey].footAngle = (base[supportKey].footAngle ?? 0) - attackPresent.supportSide * toe * 0.026;
    base[freeKey].toeFlex = Math.max(base[freeKey].toeFlex ?? 0, toe * (specPresent.kick || specPresent.sweep ? 0.48 : 0.24));
  }
  ...
  if (!winner && f.hurt > 0 && hitMass.active > 0.035 && hitMass.kind !== "guard") {
    ...
    const present = reactionPresentationProfile(f);
    if (present.active > 0.035) {
      const fingerShock = clamp(present.handShock * (heavyHit ? 1.08 : 0.92), 0, heavyHit ? 0.72 : 0.58);
...
function drawLeg(f, leg, front) {
  ...
  drawVectorFootAnatomy(spec, outfit, plant, lift, legExtension, clamp(leg.toeFlex ?? 0, 0, 0.65));
  ctx.restore();
}
...
function drawVectorFootAnatomy(spec, outfit, plant, lift, extension = 0, toeFlex = 0) {
  ctx.save();
  ctx.globalCompositeOperation = "multiply";
...
function drawVectorHandAnatomy(f, arm, spec, front) {
  ...
  for (let i = -1; i <= 1; i += 1) {
    ...
    ctx.quadraticCurveTo(
      x + (curl * 1.1 + fingerWave * 0.25) * spec.hand,
      arm.hand.y - 1.4 * spec.hand + fatigue * 1.2 + fingerFidget * 0.6,
      x + 1.2 * spec.hand + fingerWave * 0.4,
...
function drawCounterReadyFX(color, crouch, f) {
  ...
  const guardProfile = defense.guardProfile;
  const counterProfile = defense.counterProfile;
  ...
  const load = defense.counterCoil;
  const release = defense.counterSnap;
  const centerX = 30 + counter * 5 + release * 5 * counterProfile.hand;
  const centerY = -112 + crouch - release * 5 * counterProfile.lift + load * 3 * counterProfile.sink + defense.crouchSettle * 1.5;
